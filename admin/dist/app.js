...
var $author$project$Main$expectJsonWithApiError = F2(
			...
			function (response) {
				switch (response.$) {
					...
					case 'BadStatus_':
						...
						return $elm$core$Result$Err(
							$elm$http$Http$BadBody(
								A2($author$project$Main$apiErrorMessage, metadata.statusCode, body)));
					case 'BadUrl_':
						var url = response.a;
						return $elm$core$Result$Err(
...
var $author$project$Main$FormCreate = {$: 'FormCreate'};
var $author$project$Main$FormEdit = function (a) {
	return {$: 'FormEdit', a: a};
};
...
var $author$project$Main$findInputAlias = F2(
	function (aliasName, model) {
		...
		if (_v0.$ === 'Loaded') {
			...
			return $elm$core$List$head(
				A2(
					...
					function (aliasInfo) {
						return _Utils_eq(aliasInfo.name, aliasName);
					},
					schema.inputAliases));
		} else {
			return $elm$core$Maybe$Nothing;
...
var $author$project$Main$actionFormDefaults = F2(
	function (model, actionInfo) {
		var _v0 = A2($author$project$Main$findInputAlias, actionInfo.inputAlias, model);
		if (_v0.$ === 'Nothing') {
			return $elm$core$Dict$empty;
		} else {
			var aliasInfo = _v0.a;
			return $elm$core$Dict$fromList(
				A2(
					$elm$core$List$map,
					function (field) {
						return _Utils_Tuple2(field.name, '');
					},
					aliasInfo.fields));
		}
	});
...
var $author$project$Main$encodeActionField = F3(
	function (valuesByName, field, partialResult) {
		if (partialResult.$ === 'Err') {
			var message = partialResult.a;
			return $elm$core$Result$Err(message);
		} else {
			var items = partialResult.a;
			var rawValue = $elm$core$String$trim(
				A2(
					$elm$core$Maybe$withDefault,
					'',
					A2($elm$core$Dict$get, field.name, valuesByName)));
			if (rawValue === '') {
				return $elm$core$Result$Err('Field ' + (field.name + ' is required'));
			} else {
				var _v1 = field.fieldType;
				switch (_v1) {
					case 'String':
						return $elm$core$Result$Ok(
							A2(
								...
								_Utils_Tuple2(
									field.name,
									$elm$json$Json$Encode$string(rawValue)),
								items));
					case 'Int':
						var _v2 = $elm$core$String$toInt(rawValue);
						if (_v2.$ === 'Just') {
							var value = _v2.a;
							return $elm$core$Result$Ok(
								A2(
							...
							return $elm$core$Result$Err('Field ' + (field.name + ' expects Int'));
						}
					case 'Float':
						var _v3 = $elm$core$String$toFloat(rawValue);
						if (_v3.$ === 'Just') {
							var value = _v3.a;
							return $elm$core$Result$Ok(
								A2(
							...
							return $elm$core$Result$Err('Field ' + (field.name + ' expects Float'));
						}
					case 'Bool':
						var lowered = $elm$core$String$toLower(rawValue);
						return ((lowered === 'true') || ((lowered === '1') || (lowered === 'yes'))) ? $elm$core$Result$Ok(
							A2(
								$elm$core$List$cons,
								_Utils_Tuple2(
									field.name,
									$elm$json$Json$Encode$bool(true)),
								items)) : (((lowered === 'false') || ((lowered === '0') || (lowered === 'no'))) ? $elm$core$Result$Ok(
							A2(
								$elm$core$List$cons,
								_Utils_Tuple2(
									field.name,
									$elm$json$Json$Encode$bool(false)),
								items)) : $elm$core$Result$Err('Field ' + (field.name + ' expects Bool (true/false)')));
					default:
						return $elm$core$Result$Err('Unsupported input type ' + (field.fieldType + (' for field ' + field.name)));
				}
			}
		}
	});
...
var $author$project$Main$actionPayloadFromForm = F2(
	function (model, actionInfo) {
		var _v0 = A2($author$project$Main$findInputAlias, actionInfo.inputAlias, model);
		if (_v0.$ === 'Nothing') {
			return $elm$core$Result$Err('Input alias not found: ' + actionInfo.inputAlias);
		} else {
			var aliasInfo = _v0.a;
			return A2(
				$elm$core$Result$map,
				...
				A3(
					$elm$core$List$foldl,
					$author$project$Main$encodeActionField(model.actionFormValues),
...
var $author$project$Main$RequestCodeResponse = F2(
	function (message, devCode) {
		return {devCode: devCode, message: message};
	});
var $author$project$Main$requestCodeDecoder = A3(
	$elm$json$Json$Decode$map2,
	$author$project$Main$RequestCodeResponse,
	A2($elm$json$Json$Decode$field, 'message', $elm$json$Json$Decode$string),
	$elm$json$Json$Decode$oneOf(
		_List_fromArray(
			[
		...
		return $elm$http$Http$request(
			{
				...
				expect: A2(
					...
					$author$project$Main$GotBootstrapFirstAdmin(scope),
					$author$project$Main$requestCodeDecoder),
				headers: _List_Nil,
				method: 'POST',
		...
		return $elm$http$Http$request(
			{
				...
				timeout: $elm$core$Maybe$Nothing,
				tracker: $elm$core$Maybe$Nothing,
				url: _Utils_ap(model.apiBase, entity.resource)
			});
	});
var $author$project$Main$GotDelete = function (a) {
	return {$: 'GotDelete', a: a};
};
var $author$project$Main$expectUnitWithApiError = function (toMsg) {
	return A2(
		$elm$http$Http$expectStringResponse,
		toMsg,
		function (response) {
...
var $author$project$Main$deleteRowRequest = F3(
	function (model, entity, idValue) {
		return $elm$http$Http$request(
			{
				body: $elm$http$Http$emptyBody,
				expect: $author$project$Main$expectUnitWithApiError($author$project$Main$GotDelete),
				headers: $author$project$Main$appAuthHeaders(model),
				method: 'DELETE',
				timeout: $elm$core$Maybe$Nothing,
				tracker: $elm$core$Maybe$Nothing,
		...
		switch (_v0.$) {
			case 'StringType':
				return $elm$core$Result$Ok(
					$elm$json$Json$Encode$string(rawValue));
			case 'IntType':
				var _v1 = $elm$core$String$toInt(rawValue);
				if (_v1.$ === 'Just') {
					var value = _v1.a;
					return $elm$core$Result$Ok(
						$elm$json$Json$Encode$int(value));
			...
			case 'FloatType':
				var _v2 = $elm$core$String$toFloat(rawValue);
				if (_v2.$ === 'Just') {
					var value = _v2.a;
					return $elm$core$Result$Ok(
						$elm$json$Json$Encode$float(value));
			...
			default:
				var lowered = $elm$core$String$toLower(rawValue);
				return ((lowered === 'true') || ((lowered === '1') || (lowered === 'yes'))) ? $elm$core$Result$Ok(
					$elm$json$Json$Encode$bool(true)) : (((lowered === 'false') || ((lowered === '0') || (lowered === 'no'))) ? $elm$core$Result$Ok(
			...
			if (rawValue === '') {
				...
				if (_v1.$ === 'Ok') {
					...
					return $elm$core$Result$Ok(
						A2(
							$elm$core$List$cons,
							_Utils_Tuple2(field.name, encoded),
							items));
				} else {
					var message = _v1.a;
					return $elm$core$Result$Err(message);
...
var $author$project$Main$findAction = F2(
	function (actionName, model) {
		var _v0 = model.schema;
		if (_v0.$ === 'Loaded') {
			var schema = _v0.a;
			return $elm$core$List$head(
				A2(
					$elm$core$List$filter,
					function (actionInfo) {
						return _Utils_eq(actionInfo.name, actionName);
					},
					schema.actions));
		} else {
			return $elm$core$Maybe$Nothing;
		...
		if (_v0.$ === 'Loaded') {
			...
			return $elm$core$List$head(
				A2(
					$elm$core$List$filter,
					function (entity) {
						return _Utils_eq(entity.name, entityName);
					},
					schema.entities));
		} else {
			return $elm$core$Maybe$Nothing;
...
var $author$project$Main$formDefaults = function (model) {
...
var $author$project$Belm$Api$valueToString = function (value) {
	...
	if (_v0.$ === 'Ok') {
		...
		var _v1 = A2($elm$json$Json$Decode$decodeValue, $elm$json$Json$Decode$bool, value);
		if (_v1.$ === 'Ok') {
			var boolValue = _v1.a;
...
var $author$project$Main$formFromRow = F2(
	function (model, rowValue) {
		var _v0 = model.selectedEntity;
		if (_v0.$ === 'Nothing') {
			return $elm$core$Dict$empty;
		} else {
...
var $author$project$Main$VersionApp = F3(
	function (name, buildTime, manifestHash) {
		return {buildTime: buildTime, manifestHash: manifestHash, name: name};
	});
var $author$project$Main$versionAppDecoder = A4(
	$elm$json$Json$Decode$map3,
	$author$project$Main$VersionApp,
	A2($elm$json$Json$Decode$field, 'name', $elm$json$Json$Decode$string),
...
var $author$project$Main$versionBelmDecoder = A4(
	...
	A2($elm$json$Json$Decode$field, 'buildTime', $elm$json$Json$Decode$string));
var $author$project$Main$VersionRuntime = F2(
	function (goVersion, platform) {
		return {goVersion: goVersion, platform: platform};
	});
var $author$project$Main$versionRuntimeDecoder = A3(
	$elm$json$Json$Decode$map2,
	$author$project$Main$VersionRuntime,
	A2($elm$json$Json$Decode$field, 'goVersion', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'platform', $elm$json$Json$Decode$string));
var $author$project$Main$adminVersionDecoder = A4(
	$elm$json$Json$Decode$map3,
...
var $author$project$Main$loadAdminVersion = function (model) {
	return $elm$http$Http$request(
		{
			body: $elm$http$Http$emptyBody,
			expect: A2($author$project$Main$expectJsonWithApiError, $author$project$Main$GotAdminVersion, $author$project$Main$adminVersionDecoder),
			headers: $author$project$Main$appAuthHeaders(model),
			method: 'GET',
			timeout: $elm$core$Maybe$Nothing,
			tracker: $elm$core$Maybe$Nothing,
...
var $author$project$Main$backupFileDecoder = A5(
	$elm$json$Json$Decode$map4,
	$author$project$Main$BackupFile,
...
var $author$project$Main$GotPerformance = function (a) {
	return {$: 'GotPerformance', a: a};
};
var $author$project$Main$PerfPayload = F5(
	function (uptimeSeconds, goroutines, memoryBytes, sqliteBytes, http) {
		return {goroutines: goroutines, http: http, memoryBytes: memoryBytes, sqliteBytes: sqliteBytes, uptimeSeconds: uptimeSeconds};
	});
var $author$project$Main$PerfHttp = F5(
	function (totalRequests, success2xx, errors4xx, errors5xx, routes) {
		return {errors4xx: errors4xx, errors5xx: errors5xx, routes: routes, success2xx: success2xx, totalRequests: totalRequests};
	});
var $author$project$Main$PerfRoute = F6(
	function (method, route, count, errors4xx, errors5xx, avgMs) {
		return {avgMs: avgMs, count: count, errors4xx: errors4xx, errors5xx: errors5xx, method: method, route: route};
	});
var $author$project$Main$perfRouteDecoder = A7(
	$elm$json$Json$Decode$map6,
	$author$project$Main$PerfRoute,
	...
	A2($elm$json$Json$Decode$field, 'errors4xx', $elm$json$Json$Decode$int),
	A2($elm$json$Json$Decode$field, 'errors5xx', $elm$json$Json$Decode$int),
	A2($elm$json$Json$Decode$field, 'avgMs', $elm$json$Json$Decode$float));
var $author$project$Main$perfHttpDecoder = A6(
	$elm$json$Json$Decode$map5,
	$author$project$Main$PerfHttp,
	A2($elm$json$Json$Decode$field, 'totalRequests', $elm$json$Json$Decode$int),
	$elm$json$Json$Decode$oneOf(
		_List_fromArray(
			[
				A2($elm$json$Json$Decode$field, 'success2xx', $elm$json$Json$Decode$int),
	...
	A2(
		$elm$json$Json$Decode$field,
		'routes',
		$elm$json$Json$Decode$list($author$project$Main$perfRouteDecoder)));
var $author$project$Main$perfPayloadDecoder = A6(
	$elm$json$Json$Decode$map5,
	$author$project$Main$PerfPayload,
	A2($elm$json$Json$Decode$field, 'uptimeSeconds', $elm$json$Json$Decode$float),
	A2($elm$json$Json$Decode$field, 'goroutines', $elm$json$Json$Decode$int),
	A2($elm$json$Json$Decode$field, 'memoryBytes', $elm$json$Json$Decode$float),
	A2($elm$json$Json$Decode$field, 'sqliteBytes', $elm$json$Json$Decode$float),
	A2($elm$json$Json$Decode$field, 'http', $author$project$Main$perfHttpDecoder));
var $author$project$Main$loadPerformance = function (model) {
	return $elm$http$Http$request(
		{
			body: $elm$http$Http$emptyBody,
...
var $author$project$Main$GotLoginWithCode = F2(
	function (a, b) {
		return {$: 'GotLoginWithCode', a: a, b: b};
	});
var $author$project$Main$LoginResponse = F3(
					...
					$elm$json$Json$Encode$object(
						_List_fromArray(
							[
								...
								'code',
								$elm$json$Json$Encode$string(
									$elm$core$String$trim(model.authCode)))
							]))),
				expect: A2(
					$author$project$Main$expectJsonWithApiError,
					$author$project$Main$GotLoginWithCode(scope),
					$author$project$Main$loginResponseDecoder),
				headers: _List_Nil,
				method: 'POST',
				timeout: $elm$core$Maybe$Nothing,
				tracker: $elm$core$Maybe$Nothing,
...
var $author$project$Main$GotLogoutSession = F2(
	function (a, b) {
		return {$: 'GotLogoutSession', a: a, b: b};
	});
var $author$project$Main$logoutSession = F2(
	function (scope, model) {
		...
		return $elm$http$Http$request(
			{
				body: $elm$http$Http$emptyBody,
				expect: $author$project$Main$expectUnitWithApiError(
					$author$project$Main$GotLogoutSession(scope)),
				headers: headers,
				method: 'POST',
				timeout: $elm$core$Maybe$Nothing,
				tracker: $elm$core$Maybe$Nothing,
...
var $author$project$Main$preferredInitialEntity = function (schema) {
	var authEntityName = A2(
		$elm$core$Maybe$map,
		function ($) {
			return $.userEntity;
		},
		schema.auth);
	var nonAuthEntities = function () {
		if (authEntityName.$ === 'Just') {
			var entityName = authEntityName.a;
			return A2(
				$elm$core$List$filter,
				function (entity) {
					return !_Utils_eq(entity.name, entityName);
	...
	var _v0 = $elm$core$List$head(nonAuthEntities);
	if (_v0.$ === 'Just') {
		var entity = _v0.a;
		return $elm$core$Maybe$Just(entity);
	} else {
		return $elm$core$List$head(schema.entities);
	}
};
var $author$project$Main$rowId = F2(
...
var $author$project$Main$replaceRow = F3(
	function (entity, updated, rows) {
		var _v0 = A2($author$project$Main$rowId, entity, updated);
		if (_v0.$ === 'Nothing') {
			return rows;
		} else {
			var targetId = _v0.a;
			return A2(
				$elm$core$List$map,
				function (rowValue) {
					var _v1 = A2($author$project$Main$rowId, entity, rowValue);
					if (_v1.$ === 'Just') {
						var currentId = _v1.a;
						return _Utils_eq(currentId, targetId) ? updated : rowValue;
					} else {
						return rowValue;
					}
				},
...
var $author$project$Main$GotRequestAuthCode = F2(
	function (a, b) {
		return {$: 'GotRequestAuthCode', a: a, b: b};
	});
		...
		return $elm$http$Http$request(
			{
				...
				expect: A2(
					$author$project$Main$expectJsonWithApiError,
					$author$project$Main$GotRequestAuthCode(scope),
					$author$project$Main$requestCodeDecoder),
				headers: _List_Nil,
				method: 'POST',
				timeout: $elm$core$Maybe$Nothing,
...
var $author$project$Main$GotRunAction = function (a) {
	return {$: 'GotRunAction', a: a};
};
var $author$project$Main$runAction = F3(
	function (model, actionInfo, payload) {
		return $elm$http$Http$request(
			{
				body: $elm$http$Http$jsonBody(payload),
...
var $author$project$Main$hasAuthorizationError = function (rowsRemote) {
	if (rowsRemote.$ === 'Failed') {
		...
		var lowered = $elm$core$String$toLower(message);
		return A2($elm$core$String$contains, '401', message) || (A2($elm$core$String$contains, '403', message) || (A2($elm$core$String$contains, 'authentication required', lowered) || (A2($elm$core$String$contains, 'not authorized', lowered) || (A2($elm$core$String$contains, 'admin role required', lowered) || A2($elm$core$String$contains, 'forbidden', lowered)))));
	} else {
		return false;
	}
};
var $author$project$Main$isCrudScreen = function (model) {
	var hasEntitySelection = function () {
		var _v1 = model.selectedEntity;
		if (_v1.$ === 'Just') {
			return true;
		} else {
			return false;
	...
	var hasActionSelection = function () {
...
var $author$project$Main$GotBackup = function (a) {
	return {$: 'GotBackup', a: a};
};
var $author$project$Main$BackupResponse = F3(
	function (path, backupDir, removed) {
		return {backupDir: backupDir, path: path, removed: removed};
	});
var $author$project$Main$backupResponseDecoder = A4(
	$elm$json$Json$Decode$map3,
	$author$project$Main$BackupResponse,
	A2($elm$json$Json$Decode$field, 'path', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'backupDir', $elm$json$Json$Decode$string),
	$elm$json$Json$Decode$oneOf(
		_List_fromArray(
...
var $author$project$Main$triggerBackup = function (model) {
	return $elm$http$Http$request(
		{
			body: $elm$http$Http$emptyBody,
...
var $author$project$Main$GotUpdate = function (a) {
	return {$: 'GotUpdate', a: a};
};
var $author$project$Main$updateRow = F4(
	function (model, entity, idValue, payload) {
		return $elm$http$Http$request(
			{
				body: $elm$http$Http$jsonBody(payload),
	...
	function (msg, model) {
		switch (msg.$) {
			case 'GotSchema':
				...
				if (result.$ === 'Ok') {
					var schema = result.a;
					var keepAuthToolsOpen = model.authToolsOpen || (!$author$project$Main$hasActiveSession(model));
					var maybeEntity = keepAuthToolsOpen ? model.selectedEntity : $author$project$Main$preferredInitialEntity(schema);
					var shouldLoadRows = !_Utils_eq(maybeEntity, $elm$core$Maybe$Nothing);
					...
					return shouldLoadRows ? _Utils_Tuple2(
						nextModel,
						$author$project$Main$loadRows(nextModel)) : _Utils_Tuple2(nextModel, $elm$core$Platform$Cmd$none);
				} else {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								rows: $author$project$Main$Failed('schema unavailable'),
								schema: $author$project$Main$Failed(
			...
			case 'SelectAction':
				var actionName = msg.a;
				var nextAction = A2($author$project$Main$findAction, actionName, model);
				if (nextAction.$ === 'Nothing') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Action not found')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var actionInfo = nextAction.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								actionFormValues: A2($author$project$Main$actionFormDefaults, model, actionInfo),
								actionResult: $elm$core$Maybe$Nothing,
								authToolsOpen: false,
								databaseMode: false,
			...
			case 'ReloadRows':
				var nextModel = _Utils_update(
					model,
					{flash: $elm$core$Maybe$Nothing, rows: $author$project$Main$Loading});
				return _Utils_Tuple2(
					nextModel,
					$author$project$Main$loadRows(nextModel));
			case 'GotRows':
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								rows: $author$project$Main$Loaded(rows)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								rows: $author$project$Main$Failed(
									$author$project$Main$httpErrorToString(httpError))
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'SelectPerformance':
						...
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									...
									$author$project$Main$loadAdminVersion(nextModel)
								])));
				}
				...
				if (!$author$project$Main$isAdminProfile(model)) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Admin role required to access database tools.')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var nextModel = _Utils_update(
				...
				if (!$author$project$Main$isAdminProfile(model)) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Only admin can refresh database tools')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var nextModel = _Utils_update(
						model,
						{backups: $author$project$Main$Loading, flash: $elm$core$Maybe$Nothing, perf: $author$project$Main$Loading});
					return _Utils_Tuple2(
						nextModel,
						$elm$core$Platform$Cmd$batch(
							_List_fromArray(
			...
			case 'ReloadPerformance':
				var nextModel = _Utils_update(
					model,
					{adminVersion: $author$project$Main$Loading, flash: $elm$core$Maybe$Nothing, perf: $author$project$Main$Loading});
				return _Utils_Tuple2(
					nextModel,
					$elm$core$Platform$Cmd$batch(
						_List_fromArray(
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								perf: $author$project$Main$Loaded(perf)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								adminVersion: $author$project$Main$Loaded(payload)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								adminVersion: $author$project$Main$Failed(
									$author$project$Main$httpErrorToString(httpError))
							}),
						$elm$core$Platform$Cmd$none);
				}
			...
			case 'GotBackups':
				var result = msg.a;
				if (result.$ === 'Ok') {
					var backups = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								backups: $author$project$Main$Loaded(backups)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								backups: $author$project$Main$Failed(
									$author$project$Main$httpErrorToString(httpError))
							}),
						$elm$core$Platform$Cmd$none);
				}
			...
			case 'SetActionField':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							actionFormValues: A3($elm$core$Dict$insert, fieldName, value, model.actionFormValues)
						}),
					$elm$core$Platform$Cmd$none);
			case 'RequestAuthCode':
				if ($elm$core$String$trim(model.authEmail) === '') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Email is required for request-code')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{flash: $elm$core$Maybe$Nothing}),
						A2($author$project$Main$requestAuthCode, scope, model));
				}
			case 'GotRequestAuthCode':
				var scope = msg.a;
				var result = msg.b;
				if (result.$ === 'Ok') {
					...
					if (_v9.$ === 'Just') {
						...
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									authCode: code,
									flash: $elm$core$Maybe$Just('Login code sent. Development code: ' + (code + '. Enter this code and click Login.'))
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(
			...
			case 'BootstrapFirstAdmin':
				if ($elm$core$String$trim(model.authEmail) === '') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Email is required to create the first admin')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var scope = $author$project$Main$activeAuthScope(model);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{flash: $elm$core$Maybe$Nothing}),
						A2($author$project$Main$bootstrapFirstAdmin, scope, model));
				}
			case 'GotBootstrapFirstAdmin':
				var scope = msg.a;
				var result = msg.b;
				if (result.$ === 'Ok') {
					var response = result.a;
					var _v11 = response.devCode;
					if (_v11.$ === 'Just') {
						var code = _v11.a;
						var nextModel = _Utils_update(
							model,
			...
			case 'LoginWithCode':
				if (($elm$core$String$trim(model.authEmail) === '') || ($elm$core$String$trim(model.authCode) === '')) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Email and code are required for login')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var scope = $author$project$Main$activeAuthScope(model);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{flash: $elm$core$Maybe$Nothing}),
						A2($author$project$Main$loginWithCode, scope, model));
				}
			case 'GotLoginWithCode':
				var scope = msg.a;
				var result = msg.b;
				if (result.$ === 'Ok') {
					var response = result.a;
					if (scope.$ === 'AppAuthScope') {
						...
						var refreshCmd = model.performanceMode ? $elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									$author$project$Main$loadPerformance(nextModel),
									$author$project$Main$loadAdminVersion(nextModel)
								])) : (model.requestLogsMode ? $author$project$Main$loadRequestLogs(nextModel) : (model.databaseMode ? $elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									$author$project$Main$loadPerformance(nextModel),
									$author$project$Main$loadBackups(nextModel)
			...
			case 'LoadAuthMe':
				...
				var hasToken = function () {
					if (scope.$ === 'AppAuthScope') {
						return $elm$core$String$trim(model.authToken) !== '';
					} else {
						return $elm$core$String$trim(model.systemAuthToken) !== '';
					}
				}();
				return (!hasToken) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							flash: $elm$core$Maybe$Just('Provide a bearer token first')
						}),
					$elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{flash: $elm$core$Maybe$Nothing}),
					A2($author$project$Main$loadAuthMe, scope, model));
			case 'GotAuthMe':
				var scope = msg.a;
				var result = msg.b;
				if (result.$ === 'Ok') {
					...
					if (scope.$ === 'AppAuthScope') {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									currentEmail: $elm$core$Maybe$Just(response.email),
									currentRole: response.role,
									flash: $elm$core$Maybe$Just('Authenticated as ' + (response.email + roleText))
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									currentSystemEmail: $elm$core$Maybe$Just(response.email),
									currentSystemRole: response.role,
									flash: $elm$core$Maybe$Just('Admin authenticated as ' + (response.email + roleText))
								}),
							$elm$core$Platform$Cmd$none);
					}
				} else {
			...
			case 'LogoutSession':
				...
				return (!hasToken) ? _Utils_Tuple2(
					...
					_Utils_update(
						model,
						{flash: $elm$core$Maybe$Nothing}),
					A2($author$project$Main$logoutSession, scope, model));
			case 'GotLogoutSession':
				var scope = msg.a;
				var result = msg.b;
				if (result.$ === 'Ok') {
					if (scope.$ === 'AppAuthScope') {
			...
			case 'SelectAuthTab':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{authTab: tab, flash: $elm$core$Maybe$Nothing}),
					$elm$core$Platform$Cmd$none);
			case 'TriggerBackup':
				return (!$author$project$Main$isAdminProfile(model)) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							flash: $elm$core$Maybe$Just('Only admin can create backups')
						}),
					$elm$core$Platform$Cmd$none) : _Utils_Tuple2(
					_Utils_update(
						model,
						{flash: $elm$core$Maybe$Nothing}),
					$author$project$Main$triggerBackup(model));
			case 'GotBackup':
				var result = msg.a;
				if (result.$ === 'Ok') {
					var response = result.a;
					var removedCount = $elm$core$List$length(response.removed);
					var removedText = (removedCount > 0) ? (' Removed ' + ($elm$core$String$fromInt(removedCount) + ' old backup(s).')) : '';
					var nextModel = _Utils_update(
						model,
						{
							backups: $author$project$Main$Loading,
							flash: $elm$core$Maybe$Just('Backup created at ' + (response.path + ('.' + removedText))),
							lastBackup: $elm$core$Maybe$Just(response)
						});
					return _Utils_Tuple2(
						nextModel,
						$elm$core$Platform$Cmd$batch(
			...
			case 'SelectRow':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							selectedRow: $elm$core$Maybe$Just(rowValue)
						}),
					$elm$core$Platform$Cmd$none);
			case 'StartCreate':
				var defaults = $author$project$Main$formDefaults(model);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{flash: $elm$core$Maybe$Nothing, formMode: $author$project$Main$FormCreate, formValues: defaults, selectedRow: $elm$core$Maybe$Nothing}),
					$elm$core$Platform$Cmd$none);
			case 'StartEdit':
				var rowValue = msg.a;
				var defaults = A2($author$project$Main$formFromRow, model, rowValue);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							flash: $elm$core$Maybe$Nothing,
							formMode: $author$project$Main$FormEdit(rowValue),
							formValues: defaults,
							selectedRow: $elm$core$Maybe$Just(rowValue)
						}),
					$elm$core$Platform$Cmd$none);
			case 'CancelForm':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{flash: $elm$core$Maybe$Nothing, formMode: $author$project$Main$FormHidden, formValues: $elm$core$Dict$empty}),
					$elm$core$Platform$Cmd$none);
			case 'SetFormField':
				var key = msg.a;
				var value = msg.b;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							formValues: A3($elm$core$Dict$insert, key, value, model.formValues)
						}),
					$elm$core$Platform$Cmd$none);
			case 'SubmitForm':
				...
				if (_v22.$ === 'Nothing') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Select an entity first')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var entity = _v22.a;
					...
					if (_v23.$ === 'Err') {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									flash: $elm$core$Maybe$Just(message)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						var payload = _v23.a;
								...
								if (_v25.$ === 'Nothing') {
									return _Utils_Tuple2(
										_Utils_update(
											model,
											{
												flash: $elm$core$Maybe$Just('Could not find row id')
											}),
										$elm$core$Platform$Cmd$none);
								} else {
									var idValue = _v25.a;
							...
							default:
								return _Utils_Tuple2(
									_Utils_update(
										model,
										{
											flash: $elm$core$Maybe$Just('Nothing to save')
										}),
									$elm$core$Platform$Cmd$none);
						}
					}
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Created successfully'),
								formMode: $author$project$Main$FormHidden,
								formValues: $elm$core$Dict$empty,
								rows: nextRows
							}),
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Updated successfully'),
								formMode: $author$project$Main$FormHidden,
								formValues: $elm$core$Dict$empty,
								rows: nextRows,
								selectedRow: $elm$core$Maybe$Just(updatedRow)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
			...
			case 'GotDelete':
				...
				if (result.$ === 'Ok') {
					var nextModel = _Utils_update(
						model,
						{
							flash: $elm$core$Maybe$Just('Deleted successfully'),
							formMode: $author$project$Main$FormHidden,
							formValues: $elm$core$Dict$empty,
							selectedRow: $elm$core$Maybe$Nothing
						});
				...
				if (_v34.$ === 'Nothing') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								flash: $elm$core$Maybe$Just('Select an action first')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					...
					if (_v35.$ === 'Err') {
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{actionResult: $elm$core$Maybe$Nothing, flash: $elm$core$Maybe$Nothing}),
							A3($author$project$Main$runAction, model, actionInfo, payload));
					}
				}
			case 'GotRunAction':
				var result = msg.a;
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								actionResult: $elm$core$Maybe$Just(response),
								flash: $elm$core$Maybe$Just('Action executed successfully')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var httpError = result.a;
...
var $author$project$Main$badge = function (labelText) {
	return A2(
		$mdgriffith$elm_ui$Element$el,
		_List_fromArray(
			[
				$mdgriffith$elm_ui$Element$Background$color(
				A3($mdgriffith$elm_ui$Element$rgb255, 234, 240, 250)),
				$mdgriffith$elm_ui$Element$Border$rounded(999),
				$mdgriffith$elm_ui$Element$Font$size(11),
				$mdgriffith$elm_ui$Element$paddingEach(
				{bottom: 4, left: 8, right: 8, top: 4})
			]),
...
var $author$project$Main$viewPanelTitle = F2(
...
var $author$project$Main$viewPanelHeader = F3(
	function (title, details, actions) {
		var titleBlock = $elm$core$List$isEmpty(actions) ? A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$width($mdgriffith$elm_ui$Element$fill),
			...
			A2($author$project$Main$viewPanelTitle, title, details)) : A2($author$project$Main$viewPanelTitle, title, details);
		return A2(
			...
			_List_fromArray(
				[
					titleBlock,
					$elm$core$List$isEmpty(actions) ? $mdgriffith$elm_ui$Element$none : A2(
					$mdgriffith$elm_ui$Element$row,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$spacing(10)
...
var $author$project$Main$viewAuthToolsPanel = function (model) {
	if (!model.authToolsOpen) {
		...
		var needsBootstrap = function () {
			if (activeScope.$ === 'AppAuthScope') {
				...
				if (maybeSystemAuth.$ === 'Just') {
					var systemAuth = maybeSystemAuth.a;
					return systemAuth.needsBootstrap;
				} else {
					return false;
				}
			}
		...
		var transportText = function () {
			if (activeScope.$ === 'AppAuthScope') {
				if (maybeAppAuth.$ === 'Just') {
					var appAuth = maybeAppAuth.a;
					return 'Transport: ' + (appAuth.emailTransport + (' | User entity: ' + appAuth.userEntity));
				} else {
					return 'User authentication is not enabled.';
				}
			} else {
				if (maybeSystemAuth.$ === 'Just') {
					var systemAuth = maybeSystemAuth.a;
					return 'Transport: ' + (systemAuth.emailTransport + ' | Scope: admin');
				} else {
					return 'Admin authentication is not available.';
				}
			}
		}();
		var activeBadgeText = function () {
			if (activeScope.$ === 'AppAuthScope') {
				return _List_fromArray(
					[
		...
		return (!$author$project$Main$hasAnyAuthInfo(model)) ? $mdgriffith$elm_ui$Element$none : A2(
			...
			_List_fromArray(
				[
					A3(
					$author$project$Main$viewPanelHeader,
					'Authentication',
					_List_fromArray(
						[
							A2(
							...
							$mdgriffith$elm_ui$Element$text(transportText))
						]),
					_List_Nil),
					A2(
					$mdgriffith$elm_ui$Element$row,
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$size(12),
							$mdgriffith$elm_ui$Element$Font$color(
							needsBootstrap ? A3($mdgriffith$elm_ui$Element$rgb255, 106, 84, 31) : A3($mdgriffith$elm_ui$Element$rgb255, 93, 103, 120))
						]),
					$mdgriffith$elm_ui$Element$text(tabHint))
				]));
	}
};
var $author$project$Main$ReloadRows = {$: 'ReloadRows'};
var $author$project$Main$StartCreate = {$: 'StartCreate'};
var $author$project$Main$RunAction = {$: 'RunAction'};
	...
	function (model, actionInfo) {
		...
		if (_v0.$ === 'Nothing') {
			return A2(
				...
				_List_fromArray(
					[
						A3($author$project$Main$viewPanelHeader, 'Action: ' + actionInfo.name, _List_Nil, _List_Nil),
						A2(
						$mdgriffith$elm_ui$Element$paragraph,
						_List_fromArray(
							[
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$text('Input alias not found: ' + actionInfo.inputAlias)
							]))
					]));
		} else {
			var aliasInfo = _v0.a;
			var fieldInput = function (field) {
				return A2(
						...
						]),
					{
						...
						onChange: $author$project$Main$SetActionField(field.name),
						placeholder: $elm$core$Maybe$Just(
							A2(
								$mdgriffith$elm_ui$Element$Input$placeholder,
								_List_Nil,
								$mdgriffith$elm_ui$Element$text(field.fieldType))),
						text: A2(
							$elm$core$Maybe$withDefault,
							'',
							A2($elm$core$Dict$get, field.name, model.actionFormValues))
					});
			};
			return A2(
				$mdgriffith$elm_ui$Element$column,
						...
						[
							...
							_List_fromArray(
								[
									...
									$mdgriffith$elm_ui$Element$text('POST /actions/' + actionInfo.name)),
									A2(
									$mdgriffith$elm_ui$Element$el,
									_List_fromArray(
										[
									...
									$mdgriffith$elm_ui$Element$text('Input: ' + aliasInfo.name))
								]),
							_List_Nil)
						]),
					_Utils_ap(
						A2($elm$core$List$map, fieldInput, aliasInfo.fields),
						_Utils_ap(
							_List_fromArray(
								[
									A2(
										...
										[
											...
											_List_fromArray(
												[
													...
													A3($mdgriffith$elm_ui$Element$rgb255, 34, 124, 95)),
													$mdgriffith$elm_ui$Element$Font$color(
													A3($mdgriffith$elm_ui$Element$rgb255, 248, 252, 250)),
													$mdgriffith$elm_ui$Element$Border$rounded(10),
													$mdgriffith$elm_ui$Element$paddingEach(
													{bottom: 10, left: 14, right: 14, top: 10})
												]),
											{
												label: $mdgriffith$elm_ui$Element$text('Run action'),
												onPress: $elm$core$Maybe$Just($author$project$Main$RunAction)
											})
										]))
								]),
							_List_fromArray(
																...
																[
																	...
																	_List_fromArray(
																		[
																			$mdgriffith$elm_ui$Element$Font$size(13),
																			$mdgriffith$elm_ui$Element$Font$color(
																			A3($mdgriffith$elm_ui$Element$rgb255, 82, 92, 108))
																		]),
																	_List_fromArray(
																		[
																			$mdgriffith$elm_ui$Element$text(
...
var $author$project$Main$SelectRow = function (a) {
	return {$: 'SelectRow', a: a};
};
...
var $author$project$Main$viewRowCard = F2(
	function (entity, rowValue) {
		var previewFields = A2($elm$core$List$take, 4, entity.fields);
		var summary = A2(
			$elm$core$String$join,
			'  |  ',
			A2(
		...
		var idText = A2(
			$elm$core$Maybe$withDefault,
			'?',
			A2($author$project$Main$rowId, entity, rowValue));
		return A2(
			$mdgriffith$elm_ui$Element$row,
			_List_fromArray(
				[
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[$mdgriffith$elm_ui$Element$Font$bold]),
							$mdgriffith$elm_ui$Element$text(entity.name + (' #' + idText))),
							A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$Font$size(13),
									$mdgriffith$elm_ui$Element$Font$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 90, 103, 120))
								]),
							$mdgriffith$elm_ui$Element$text(summary))
						])),
					A2(
					$mdgriffith$elm_ui$Element$Input$button,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 222, 232, 248)),
							$mdgriffith$elm_ui$Element$Border$rounded(8),
							$mdgriffith$elm_ui$Element$paddingEach(
							{bottom: 8, left: 10, right: 10, top: 8})
						]),
					{
						label: $mdgriffith$elm_ui$Element$text('View'),
						onPress: $elm$core$Maybe$Just(
							$author$project$Main$SelectRow(rowValue))
					}),
					A2(
					$mdgriffith$elm_ui$Element$Input$button,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Background$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 223, 244, 238)),
							$mdgriffith$elm_ui$Element$Border$rounded(8),
							$mdgriffith$elm_ui$Element$paddingEach(
							{bottom: 8, left: 10, right: 10, top: 8})
						]),
					{
						...
						onPress: $elm$core$Maybe$Just(
							$author$project$Main$StartEdit(rowValue))
					}),
					A2(
					$mdgriffith$elm_ui$Element$Input$button,
					_List_fromArray(
						...
						]),
					{
						...
						onPress: $elm$core$Maybe$Just(
							$author$project$Main$DeleteRow(rowValue))
					})
				]));
	});
var $author$project$Main$viewRows = function (model) {
	var _v0 = _Utils_Tuple2(model.selectedEntity, model.rows);
	if (_v0.a.$ === 'Nothing') {
		var _v1 = _v0.a;
		return A2(
			$mdgriffith$elm_ui$Element$paragraph,
			...
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$text('Choose an entity from the sidebar.')
				]));
	} else {
		switch (_v0.b.$) {
			case 'NotAsked':
				...
				return A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('No data loaded yet.')
						]));
			case 'Loading':
				var _v3 = _v0.b;
				return A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('Loading records...')
						]));
			case 'Failed':
				var message = _v0.b.a;
				return A2(
			...
			default:
				var entity = _v0.a.a;
				var records = _v0.b.a;
				return $elm$core$List$isEmpty(records) ? A2(
					$mdgriffith$elm_ui$Element$paragraph,
					_List_Nil,
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('No records yet.')
						])) : A2(
					$mdgriffith$elm_ui$Element$column,
					_List_fromArray(
						[
					...
					A2(
						$elm$core$List$map,
						$author$project$Main$viewRowCard(entity),
						records));
		}
	}
};
var $author$project$Main$viewDataPanel = function (model) {
	var _v0 = model.selectedAction;
	if (_v0.$ === 'Just') {
		var actionInfo = _v0.a;
		return A2($author$project$Main$viewActionPanel, model, actionInfo);
	} else {
					...
					_List_fromArray(
						[
								...
								]),
							{
								...
								onPress: $elm$core$Maybe$Just($author$project$Main$StartCreate)
							}),
							A2(
							$mdgriffith$elm_ui$Element$Input$button,
							_List_fromArray(
								...
								]),
							{
								...
								onPress: $elm$core$Maybe$Just($author$project$Main$ReloadRows)
							})
						])),
...
var $author$project$Main$formatBytes = function (bytes) {
	return (bytes < 1024) ? ($elm$core$String$fromInt(
		$elm$core$Basics$round(bytes)) + ' B') : ((_Utils_cmp(bytes, 1024 * 1024) < 0) ? ($elm$core$String$fromFloat(
		$author$project$Main$roundTo1(bytes / 1024)) + ' KB') : ((_Utils_cmp(bytes, (1024 * 1024) * 1024) < 0) ? ($elm$core$String$fromFloat(
		$author$project$Main$roundTo1(bytes / (1024 * 1024))) + ' MB') : ($elm$core$String$fromFloat(
		$author$project$Main$roundTo1(bytes / ((1024 * 1024) * 1024))) + ' GB')));
};
var $author$project$Main$backupRow = function (backup) {
	return A2(
		$mdgriffith$elm_ui$Element$row,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$text(backup.createdAt)),
				A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
					[
				...
				$mdgriffith$elm_ui$Element$text(
					$author$project$Main$formatBytes(backup.sizeBytes))),
				A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
					[
...
var $author$project$Main$databaseBackupDir = function (databasePath) {
	...
	var segments = A2($elm$core$String$split, '/', slashPath);
	var folderSegments = ($elm$core$List$length(segments) <= 1) ? _List_fromArray(
		['.']) : A2(
		$elm$core$List$take,
		$elm$core$List$length(segments) - 1,
		segments);
	var folderPath = A2($elm$core$String$join, '/', folderSegments);
	return ((cleaned === '') || (cleaned === '-')) ? '-' : ((folderPath === '') ? './backups' : (folderPath + '/backups'));
};
var $author$project$Main$databaseInfoCard = F2(
				...
				[
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$size(13),
							$mdgriffith$elm_ui$Element$Font$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 41, 52, 68))
						]),
					_List_fromArray(
						[
...
var $author$project$Main$performanceCard = F2(
	function (title, value) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[$mdgriffith$elm_ui$Element$Font$bold]),
						$mdgriffith$elm_ui$Element$text('Last backup')),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_fromArray(
							[
						...
						$mdgriffith$elm_ui$Element$text(
							'Removed old backups: ' + $elm$core$String$fromInt(
								$elm$core$List$length(backup.removed))))
					]));
		} else {
			return A2(
				$mdgriffith$elm_ui$Element$paragraph,
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('No backup executed in this admin session yet.')
					]));
		}
	}();
		...
		switch (_v1.$) {
			case 'NotAsked':
				return A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('Backups were not loaded yet.')
						]));
			case 'Loading':
				return A2(
					$mdgriffith$elm_ui$Element$paragraph,
			...
			default:
				...
				return $elm$core$List$isEmpty(payload.backups) ? A2(
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$text('No backups found yet.')
						])) : A2(
					$mdgriffith$elm_ui$Element$column,
					_List_fromArray(
						[
							...
							[
								...
								_List_fromArray(
									[
										...
										$mdgriffith$elm_ui$Element$Background$color(
										A3($mdgriffith$elm_ui$Element$rgb255, 244, 247, 252)),
										$mdgriffith$elm_ui$Element$Border$rounded(8)
									]),
								_List_fromArray(
									[
										...
										$mdgriffith$elm_ui$Element$text('Backup time')),
										A2(
										$mdgriffith$elm_ui$Element$el,
										_List_fromArray(
											[
			...
			[
				...
				_List_fromArray(
					[
						...
						{
							label: $mdgriffith$elm_ui$Element$text('Refresh'),
							onPress: $elm$core$Maybe$Just($author$project$Main$ReloadDatabase)
						}),
						A2(
						$mdgriffith$elm_ui$Element$Input$button,
						_List_fromArray(
						...
						{
							label: $mdgriffith$elm_ui$Element$text('Create backup'),
							onPress: $elm$core$Maybe$Just($author$project$Main$TriggerBackup)
						})
					])),
				A2(
				$mdgriffith$elm_ui$Element$row,
				...
				$mdgriffith$elm_ui$Element$text('Available backups')),
				backupsSection
			]));
};
var $author$project$Main$viewDatabasePanel = function (model) {
	return (!$author$project$Main$isAdminProfile(model)) ? A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('Admin role required to view database and backup information.')
					]))
			])) : $author$project$Main$viewDatabasePanelAdmin(model);
};
var $author$project$Main$ClearFlash = {$: 'ClearFlash'};
...
var $author$project$Main$viewActionInfo = function (actionInfo) {
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				$mdgriffith$elm_ui$Element$text('Action details')),
				A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$author$project$Main$badge('ACTION'),
						$author$project$Main$badge('POST')
					])),
				A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text(actionInfo.inputAlias))
					])),
				A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text(
							$elm$core$String$fromInt(actionInfo.steps)))
					]))
			]));
};
...
var $author$project$Main$viewEntitySchema = function (model) {
	var rowsForEntity = function () {
		var _v0 = model.selectedEntity;
		if (_v0.$ === 'Nothing') {
			return _List_Nil;
		} else {
		...
		_Utils_ap(
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$text('Schema'))
				]),
			A2(
				$elm$core$List$map,
				function (field) {
...
var $author$project$Main$CancelForm = {$: 'CancelForm'};
var $author$project$Main$SetFormField = F2(
	function (a, b) {
		return {$: 'SetFormField', a: a, b: b};
	});
var $author$project$Main$SubmitForm = {$: 'SubmitForm'};
var $author$project$Main$formCard = F3(
	function (model, entity, titleText) {
		var formFields = A2(
			$elm$core$List$filter,
			function (field) {
				return !field.primary;
			...
			return A2(
					...
					]),
				{
					label: A2(
						...
						$mdgriffith$elm_ui$Element$text(field.name)),
					onChange: $author$project$Main$SetFormField(field.name),
					placeholder: $elm$core$Maybe$Just(
						A2(
							$mdgriffith$elm_ui$Element$Input$placeholder,
							_List_Nil,
					...
					text: A2(
						$elm$core$Maybe$withDefault,
						'',
						A2($elm$core$Dict$get, field.name, model.formValues))
				});
		};
		return A2(
			$mdgriffith$elm_ui$Element$column,
			...
			_Utils_ap(
				...
				_Utils_ap(
					A2($elm$core$List$map, fieldInput, formFields),
					_List_fromArray(
						[
							A2(
							$mdgriffith$elm_ui$Element$row,
								...
								[
									...
									_List_fromArray(
										[
											...
											A3($mdgriffith$elm_ui$Element$rgb255, 34, 124, 95)),
											$mdgriffith$elm_ui$Element$Font$color(
											A3($mdgriffith$elm_ui$Element$rgb255, 247, 252, 249)),
											$mdgriffith$elm_ui$Element$Border$rounded(8),
											$mdgriffith$elm_ui$Element$paddingEach(
											{bottom: 10, left: 12, right: 12, top: 10})
										]),
									{
										...
										onPress: $elm$core$Maybe$Just($author$project$Main$SubmitForm)
									}),
									A2(
									$mdgriffith$elm_ui$Element$Input$button,
									_List_fromArray(
										[
											$mdgriffith$elm_ui$Element$Background$color(
											A3($mdgriffith$elm_ui$Element$rgb255, 233, 236, 242)),
											$mdgriffith$elm_ui$Element$Border$rounded(8),
											$mdgriffith$elm_ui$Element$paddingEach(
											{bottom: 10, left: 12, right: 12, top: 10})
										]),
									{
										...
										onPress: $elm$core$Maybe$Just($author$project$Main$CancelForm)
									})
								]))
						]))));
	});
var $author$project$Main$viewFormPanel = function (model) {
	var _v0 = _Utils_Tuple2(model.selectedEntity, model.formMode);
	_v0$2:
	while (true) {
		if (_v0.a.$ === 'Just') {
			switch (_v0.b.$) {
				case 'FormCreate':
					var entity = _v0.a.a;
					var _v1 = _v0.b;
					return A3($author$project$Main$formCard, model, entity, 'Create record');
				case 'FormEdit':
					var entity = _v0.a.a;
					return A3($author$project$Main$formCard, model, entity, 'Edit record');
				default:
					break _v0$2;
			}
		} else {
...
var $author$project$Main$viewSelectedRow = function (model) {
	var _v0 = model.selectedRow;
	if (_v0.$ === 'Nothing') {
		return $mdgriffith$elm_ui$Element$none;
	} else {
		var rowValue = _v0.a;
			...
			_Utils_ap(
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text('Record detail'))
					]),
				A2(
					$elm$core$List$map,
					function (_v1) {
					...
					$elm$core$Dict$toList(rowValue))));
	}
};
var $author$project$Main$viewInspector = function (model) {
	var _v0 = model.selectedAction;
	if (_v0.$ === 'Just') {
		var actionInfo = _v0.a;
		return A2(
			...
			_List_fromArray(
				[
					$author$project$Main$viewActionInfo(actionInfo)
				]));
	} else {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			...
			_List_fromArray(
				[
					$author$project$Main$viewEntitySchema(model),
					$author$project$Main$viewFormPanel(model),
					$author$project$Main$viewSelectedRow(model)
				]));
	}
};
var $author$project$Main$ReloadPerformance = {$: 'ReloadPerformance'};
var $author$project$Main$formatMs = function (ms) {
	return $elm$core$String$fromFloat(
		$author$project$Main$roundTo1(ms)) + ' ms';
};
var $author$project$Main$formatSeconds = function (seconds) {
	return $elm$core$String$fromFloat(
		$author$project$Main$roundTo1(seconds)) + ' s';
};
var $mdgriffith$elm_ui$Internal$Model$Px = function (a) {
	return {$: 'Px', a: a};
};
var $mdgriffith$elm_ui$Element$px = $mdgriffith$elm_ui$Internal$Model$Px;
var $author$project$Main$viewMonitoringVersion = function (versionRemote) {
	switch (versionRemote.$) {
		case 'NotAsked':
			return $mdgriffith$elm_ui$Element$none;
		case 'Loading':
			return A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('Loading version info...')
					]));
		case 'Failed':
			var message = versionRemote.a;
			return A2(
				$mdgriffith$elm_ui$Element$paragraph,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('Version info unavailable: ' + message)
					]));
		default:
			var versionPayload = versionRemote.a;
			return A2(
				$mdgriffith$elm_ui$Element$column,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($author$project$Main$databaseInfoCard, 'App', versionPayload.app.name),
								A2($author$project$Main$databaseInfoCard, 'App build time', versionPayload.app.buildTime),
								A2($author$project$Main$databaseInfoCard, 'Manifest hash', versionPayload.app.manifestHash)
							])),
						A2(
						$mdgriffith$elm_ui$Element$row,
						_List_fromArray(
						...
						_List_fromArray(
							[
								A2($author$project$Main$databaseInfoCard, 'Go version', versionPayload.runtimeInfo.goVersion),
								A2($author$project$Main$databaseInfoCard, 'Platform', versionPayload.runtimeInfo.platform)
							]))
					]));
	}
};
var $author$project$Main$viewPerformancePanel = function (model) {
	if (!$author$project$Main$isAdminProfile(model)) {
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
		...
		var routeRow = function (perfRoute) {
			var hasErrors = (perfRoute.errors4xx > 0) || (perfRoute.errors5xx > 0);
			var statusColor = hasErrors ? A3($mdgriffith$elm_ui$Element$rgb255, 176, 60, 46) : A3($mdgriffith$elm_ui$Element$rgb255, 34, 124, 95);
			return A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$text(perfRoute.method)),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_fromArray(
							[
						...
						$mdgriffith$elm_ui$Element$text(perfRoute.route)),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_fromArray(
							[
						...
						$mdgriffith$elm_ui$Element$text(
							'count: ' + $elm$core$String$fromInt(perfRoute.count))),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_fromArray(
							[
						...
						$mdgriffith$elm_ui$Element$text(
							'avg: ' + $author$project$Main$formatMs(perfRoute.avgMs))),
						A2(
						$mdgriffith$elm_ui$Element$el,
						_List_fromArray(
							[
						...
						$mdgriffith$elm_ui$Element$text(
							'4xx/5xx: ' + ($elm$core$String$fromInt(perfRoute.errors4xx) + ('/' + $elm$core$String$fromInt(perfRoute.errors5xx)))))
					]));
		};
		var cards = function (perf) {
			return A2(
				$mdgriffith$elm_ui$Element$row,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						A2(
						$author$project$Main$performanceCard,
						'Uptime',
						$author$project$Main$formatSeconds(perf.uptimeSeconds)),
						A2(
						$author$project$Main$performanceCard,
						'Memory (heap)',
						$author$project$Main$formatBytes(perf.memoryBytes)),
						A2(
						$author$project$Main$performanceCard,
						'SQLite file',
						$author$project$Main$formatBytes(perf.sqliteBytes)),
						A2(
						$author$project$Main$performanceCard,
						'Goroutines',
						$elm$core$String$fromInt(perf.goroutines)),
						A2(
						$author$project$Main$performanceCard,
						'Requests',
						$elm$core$String$fromInt(perf.http.totalRequests))
					]));
		};
		return A2(
			$mdgriffith$elm_ui$Element$column,
				...
				[
					...
					_List_fromArray(
						[
							...
							{
								label: $mdgriffith$elm_ui$Element$text('Refresh'),
								onPress: $elm$core$Maybe$Just($author$project$Main$ReloadPerformance)
							})
						])),
					$author$project$Main$viewMonitoringVersion(model.adminVersion),
					A2(
					$mdgriffith$elm_ui$Element$el,
					_List_fromArray(
						[
									...
									[
										...
										_List_fromArray(
											[
												...
												$author$project$Main$performanceCard,
												'2xx responses',
												$elm$core$String$fromInt(perf.http.success2xx)),
												A2(
												$author$project$Main$performanceCard,
												'4xx errors',
												$elm$core$String$fromInt(perf.http.errors4xx)),
												A2(
												$author$project$Main$performanceCard,
												'5xx errors',
												$elm$core$String$fromInt(perf.http.errors5xx))
											])),
										A2(
										$mdgriffith$elm_ui$Element$column,
										_List_fromArray(
										...
										_Utils_ap(
											_List_fromArray(
												[
													...
													$mdgriffith$elm_ui$Element$text('Route metrics'))
												]),
											$elm$core$List$isEmpty(perf.http.routes) ? _List_fromArray(
												[
													A2(
													$mdgriffith$elm_ui$Element$paragraph,
													_List_Nil,
												...
												]) : A2($elm$core$List$map, routeRow, perf.http.routes)))
									]));
					}
				}()
				]));
...
var $author$project$Main$viewRequestLogEntry = function (entry) {
	...
	var querySummary = $elm$core$String$fromInt(entry.queryCount) + (' ' + (queryLabel + (', ' + $author$project$Main$formatMs(entry.queryTimeMs))));
...
var $author$project$Main$viewRequestLogsPanel = function (model) {
	if (!$author$project$Main$isAdminProfile(model)) {
		...
		var logsSubtitle = function () {
			var _v0 = model.requestLogs;
			if (_v0.$ === 'Loaded') {
				var payload = _v0.a;
				return 'Showing ' + ($elm$core$String$fromInt(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$author$project$Main$viewPanelHeader,
					'Recent request logs',
					(logsSubtitle === '') ? _List_Nil : _List_fromArray(
						[
							A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
							...
							$mdgriffith$elm_ui$Element$text(logsSubtitle))
						]),
					_List_fromArray(
						[
							A2(
...
var $author$project$Main$viewContent = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$Main$viewAuthToolsPanel(model),
...
var $author$project$Main$SelectAction = function (a) {
	return {$: 'SelectAction', a: a};
};
var $author$project$Main$SelectDatabase = {$: 'SelectDatabase'};
var $author$project$Main$SelectEntity = function (a) {
...
var $author$project$Main$splitEntitiesForSidebar = F2(
		...
		if (_v0.$ === 'Just') {
			...
			return _Utils_Tuple2(
				A2(
					$elm$core$List$filter,
					function (entity) {
						return _Utils_eq(entity.name, authInfo.userEntity);
					},
					entities),
				A2(
					$elm$core$List$filter,
			...
			return _Utils_Tuple2(_List_Nil, entities);
		}
	});
var $author$project$Main$viewSidebar = function (model) {
	...
	var entityButton = function (entity) {
		var selected = (!model.authToolsOpen) && ((!model.performanceMode) && ((!model.requestLogsMode) && ((!model.databaseMode) && function () {
			...
			if (_v5.$ === 'Just') {
				var current = _v5.a;
				return _Utils_eq(current.name, entity.name);
			} else {
				return false;
			}
		}())));
	...
	var databaseButton = function () {
		var backgroundColor = (model.databaseMode && (!model.authToolsOpen)) ? A3($mdgriffith$elm_ui$Element$rgb255, 54, 94, 217) : A3($mdgriffith$elm_ui$Element$rgb255, 24, 29, 36);
		return A2(
			$mdgriffith$elm_ui$Element$Input$button,
			_List_fromArray(
				[
	...
	var authToolsButton = function () {
		var backgroundColor = model.authToolsOpen ? A3($mdgriffith$elm_ui$Element$rgb255, 54, 94, 217) : A3($mdgriffith$elm_ui$Element$rgb255, 24, 29, 36);
		return A2(
			$mdgriffith$elm_ui$Element$Input$button,
			_List_fromArray(
				[
	...
	var actionEndpointCard = function (actionInfo) {
		var selected = (!model.authToolsOpen) && ((!model.performanceMode) && ((!model.requestLogsMode) && ((!model.databaseMode) && function () {
			...
			if (_v4.$ === 'Just') {
				var current = _v4.a;
				return _Utils_eq(current.name, actionInfo.name);
			} else {
				return false;
			}
		}())));
	...
	var _v0 = function () {
		var _v1 = model.schema;
		if (_v1.$ === 'Loaded') {
			var schema = _v1.a;
			var _v2 = A2($author$project$Main$splitEntitiesForSidebar, model, schema.entities);
			var authOnly = _v2.a;
			var crudOnly = _v2.b;
			return _Utils_Tuple3(authOnly, crudOnly, schema.actions);
		} else {
			return _Utils_Tuple3(_List_Nil, _List_Nil, _List_Nil);
		}
	}();
	var authEntities = _v0.a;
	var crudEntities = _v0.b;
	var actions = _v0.c;
	return A2(
		$mdgriffith$elm_ui$Element$column,
		_List_fromArray(
		...
		_Utils_ap(
				...
				[
					...
					_List_fromArray(
						[
							$mdgriffith$elm_ui$Element$Font$size(13),
							$mdgriffith$elm_ui$Element$Font$color(
							A3($mdgriffith$elm_ui$Element$rgb255, 144, 158, 179))
						]),
					$mdgriffith$elm_ui$Element$text(
						function () {
							var _v3 = model.schema;
							if (_v3.$ === 'Loaded') {
								var schema = _v3.a;
								return schema.appName;
							} else {
			...
			_Utils_ap(
				$author$project$Main$hasAnyAuthInfo(model) ? _Utils_ap(
					_List_fromArray(
						[
							A2(
							$mdgriffith$elm_ui$Element$el,
							_List_fromArray(
								[
									...
									$mdgriffith$elm_ui$Element$Font$bold,
									$mdgriffith$elm_ui$Element$Font$color(
									A3($mdgriffith$elm_ui$Element$rgb255, 118, 136, 160))
								]),
							$mdgriffith$elm_ui$Element$text('AUTH')),
							authToolsButton
						]),
					A2($elm$core$List$map, entityButton, authEntities)) : _List_Nil,
				_Utils_ap(
					$elm$core$List$isEmpty(crudEntities) ? _List_Nil : _Utils_ap(
						_List_fromArray(
							[
								A2(
								$mdgriffith$elm_ui$Element$el,
								...
								$mdgriffith$elm_ui$Element$text('CRUD'))
							]),
						A2($elm$core$List$map, entityButton, crudEntities)),
					_Utils_ap(
						$elm$core$List$isEmpty(actions) ? _List_Nil : _Utils_ap(
							_List_fromArray(
								[
									A2(
									$mdgriffith$elm_ui$Element$el,
							...
							A2($elm$core$List$map, actionEndpointCard, actions)),
						$author$project$Main$isAdminProfile(model) ? _List_fromArray(
							[
								A2(
								$mdgriffith$elm_ui$Element$el,
								_List_fromArray(
		...
		body: _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Background$color(
						A3($mdgriffith$elm_ui$Element$rgb255, 244, 245, 247)),
						$mdgriffith$elm_ui$Element$Font$family(
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$Font$typeface('Space Grotesk'),
								$mdgriffith$elm_ui$Element$Font$typeface('IBM Plex Sans'),
								$mdgriffith$elm_ui$Element$Font$sansSerif
							])),
						$mdgriffith$elm_ui$Element$Font$color(
						A3($mdgriffith$elm_ui$Element$rgb255, 29, 36, 44))
					]),
