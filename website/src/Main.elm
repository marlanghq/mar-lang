        ...
        Just sectionId ->
            case sectionId of
                ...
                "compiler-and-runtime-commands" ->
                    "Compiler and runtime commands. mar init store-app. mar dev store.mar. mar compile store.mar. mar fly init store.mar. mar fly deploy store.mar. mar format store.mar. mar completion zsh. mar lsp."
...
docSearchRouteText route =
    case route of
        ...
        Examples ->
            "Examples. Todo API. Minimal CRUD example. todo.mar. BookStore API. Auth, roles, and transactional action. store.mar. placeBookOrder. Examples of Mar apps."
...
docSearchEntries =
    ...
    , { title = "Compiler and runtime commands"
      ...
      , keywords = [ "mar init", "mar dev", "mar compile", "mar fly init", "mar fly deploy", "mar format", "mar completion", "mar lsp", "version" ]
      }
    ...
    column
        ...
        , panel
            ...
            , anchoredSection "compiler-and-runtime-commands"
                ...
                , commandRow model "2" "Dev" "Runs the app in development mode with hot reload when the .mar file changes." "mar dev store.mar"
                , commandRow model "3" "Compile" "Packages self-contained executables for all supported platforms and generates frontend clients." "mar compile store.mar"
                , commandRow model "4" "Fly init" "Prepares Fly.io deployment files for your app." "mar fly init store.mar"
                , commandRow model "5" "Fly deploy" "Rebuilds the Linux executable for the current app and runs fly deploy with the generated Fly config." "mar fly deploy store.mar"
                , commandRow model "6" "Format" "Applies Mar's official formatting style to source files." "mar format store.mar"
                , commandRow model "7" "Completion" "Generates shell completion scripts for terminals such as zsh." "mar completion zsh"
                , commandRow model "8" "LSP" "Starts the language server used by the VSCode extension for diagnostics, hovers, and navigation. Usually started by the editor plugin." "mar lsp"
                , paragraphWithEmphasis
