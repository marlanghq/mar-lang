...
func MarModule() string {
	...
	b.WriteString("module Frontend.Reference.Data exposing (Entry, Section, Group, modules, moduleGroups, sectionsFor, entriesFor, blurbFor)\n\n\n")
	...
	for _, mod := range Modules {
		...
		for si, s := range secs {
			...
			for ei, e := range s.Entries {
				...
				b.WriteString(fmt.Sprintf("%s{ moduleName = %q, name = %q, signature = %q, description = %q, examples = %s }\n",
					entLead, e.Module, e.Name, e.Signature, e.Desc, marNestedStringList(displayExamples(e.Examples))))
			}
			b.WriteString("                    ]\n")
			b.WriteString("              }\n")
		}
