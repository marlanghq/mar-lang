...
var appCategories = map[string][]CatGroup{
	...
	"Sound": {
		...
		{"Shape a sound", []string{"attack", "decay", "release", "sweep", "sweepBy", "holdPitch", "vibrato", "tremolo", "detune", "duty", "arp", "volume", "gain", "pan", "lowCut", "highCut"}},
		{"Move a whole passage", []string{"transpose", "stretch"}},
...
var appDescriptions = map[string]string{
	...
	"Canvas.Rotate":        "Turns everything in the group clockwise around the group origin, by an Angle from Math.degrees, Math.deciDegrees, or Math.turns.",
	...
	// Sound
	"Sound.tone":      "A note: a wave shape, a frequency in Hz, and a length in milliseconds. Everything else in this module either builds one of these or reshapes it. Anything shorter than 20 ms is played as 20 ms, which is about where a click becomes a pitch.",
	...
	"Sound.pan":       "Places the sound in the stereo image, from -100 hard left through 0 in the middle to 100 hard right. Two parts written in the same register stay muddy in the middle however good the waves are, and a third of the way apart both become audible on their own. This moves every voice of the sound, so a whole melody travels together instead of one note of it.",
	...
	"Sound.sweep":     "Glides the pitch to an end frequency over the length of the note. Sweeping down is a laser or a falling bomb, sweeping up is a power-up. Every voice lands on the same frequency, which is what you want for an effect and not for a chord: for that, use sweepBy. A bend belongs to a note with an end, so it is ignored on a voice that arpeggiates and on a held one.",
	"Sound.sweepBy":   "Bends the pitch by a distance in cents rather than to a fixed frequency, so a chord keeps its intervals on the way. 100 cents is a semitone, 1200 an octave. A rest has no pitch and is left alone.",
	...
	"Sound.vibrato":   "Wobbles the pitch, given a depth in cents and a rate in Hz. Around 10 cents at 6 Hz is a played note; past about 50 it stops being expression and becomes a siren.",
	"Sound.tremolo":   "Wobbles the loudness, given a depth as a percentage and a rate in Hz. Tremolo is to volume what vibrato is to pitch: an electric piano, a throbbing pad, a warning light in sound. The wobble only ever takes away, so the note still peaks at the level it was written at, and a depth of 100 dips all the way to silence.",
	"Sound.duty":      "The width of a square wave pulse, as a percentage. 50 is the hollow default, and moving away from it thins the tone. Square only: a pulse width is a property of that wave, so this does nothing to the other four.",
	"Sound.arp":       "Steps the pitch through a list of semitone offsets over the life of the note, so [ 4, 7 ] is a major chord above whatever note it is given. The chiptune trick for a chord from a single voice. Because the offsets are relative, the figure follows the note through a transpose. One voice can arpeggiate or bend but not both: a voice with an arpeggio ignores sweep and holdPitch.",
	"Sound.volume":    "Sets the level of every voice in this sound, 0 to 100, without touching anything else that is playing. It is an absolute level, so it flattens whatever balance the parts already had: to lower a mix and keep its balance, use gain.",
	"Sound.gain":      "Scales the level of every voice in this sound by a percentage of what it already is, so the balance inside it survives. 50 brings a whole arrangement down by half; 150 pushes it up. This is the one to reach for when a part is too loud against the rest.",
	...
	"Sound.chord":     "Plays several sounds at the same instant, as one sound. The parts may be different lengths: the chord lasts as long as its longest voice, and a shorter one simply finishes early.",
	...
	"Sound.loop":      "Repeats a sound for as long as the subscription is returned. Stop returning it and the loop ends. One cycle is the sound's own full length, so layers of different lengths stay in phase with each other pass after pass, and the shorter ones leave a gap before the cycle comes round.",
	"Sound.voice":     "Holds one voice for as long as the subscription is returned, without ever restarting it. A voice's pitch is part of what makes it that voice, so returning two pitches sounds two notes at once: this is what a keyboard wants, one voice per held key. What a held voice cannot have is anything that describes how one note changes over its own length, because it has no length: sweep and arp are ignored, and decay is honoured as a level to settle at but not as a time to fall over.",
	"Sound.glide":     "Holds one voice whose pitch is a live setting rather than part of its identity, so handing it a new pitch slides the running note there instead of starting a second one. An engine that rises with speed, a siren. Only ever one note at a time, the way glide works on a real synth. Like voice, it ignores sweep and arp: the pitch is yours to move, so a scheduled bend would only fight the next slide.",
	"Sound.transpose": "Moves an entire passage by a number of semitones, up or down. 12 is an octave. Everything that has a pitch travels together, including where a sweep is heading, and anything that has none, like noise and rests, stays as it is. The way to write a piece once and play it in another key.",
	"Sound.stretch":   "Plays an entire passage at a percentage of its speed. 50 is half speed, 200 is double. Every duration in it scales together, so the notes, the gaps between them, and their envelopes all stay in proportion, and the piece keeps its shape instead of playing long notes at the old spacing.",
	"Sound.master":    "Sets the overall output volume, 0 to 100, for everything the app plays. 100 is the level an app starts at, so a settings slider that begins at the top begins where the sound was mixed.",
	"Sound.setMuted":  "Silences or unsilences all audio, leaving the volume setting untouched. It is a mute, not a stop: a loop keeps its place while silent and is heard where it has got to, and a sound started while muted starts anyway.",
...
var appExamples = map[string][]string{
	...
	"Canvas.Rotate":        {"Canvas.group [ Canvas.Rotate (Math.degrees 90) ] [ Canvas.rect 0 0 16 4 (Canvas.rgb 255 255 255) ]"},
	...
	"Sound.pan":       {"Sound.pan 35 (Sound.tone Sound.Triangle (Sound.a 3) 500)", "Sound.pan (0 - 40) (Sound.sequence [ Sound.tone Sound.Square (Sound.c 4) 200, Sound.tone Sound.Square (Sound.e 4) 200 ])"},
	...
	"Sound.tremolo":   {"Sound.tremolo 45 6 (Sound.tone Sound.Sine (Sound.a 3) 1200)"},
	"Sound.duty":      {"Sound.duty 25 (Sound.tone Sound.Square (Sound.c 5) 120)"},
	"Sound.arp":       {"Sound.arp [ 4, 7 ] (Sound.tone Sound.Square (Sound.c 4) 240)"},
	"Sound.volume":    {"Sound.volume 40 (Sound.tone Sound.Noise 200 60)"},
	"Sound.gain":      {"Sound.gain 60 (Sound.chord [ Sound.volume 80 (Sound.tone Sound.Square (Sound.c 4) 300), Sound.volume 40 (Sound.tone Sound.Triangle (Sound.c 3) 300) ])"},
	"Sound.sweepBy":   {"Sound.sweepBy (0 - 200) (Sound.tone Sound.Sawtooth (Sound.a 3) 300)"},
	"Sound.transpose": {"Sound.transpose 5 (Sound.sequence [ Sound.tone Sound.Square (Sound.c 4) 120, Sound.tone Sound.Square (Sound.e 4) 120 ])"},
	"Sound.stretch":   {"Sound.stretch 50 (Sound.sequence [ Sound.tone Sound.Square (Sound.c 4) 120, Sound.tone Sound.Square (Sound.g 4) 120 ])"},
