...
var descriptions = map[string]string{
	...
	"Basics.always":      "Ignores its second argument and returns the first. `always x` is the constant function `\\_ -> x`, for the places that want a function where the argument does not matter.",
...
var examples = map[string][]string{
	...
	"Basics.always":      {"List.map (always 0) [ 1, 2, 3 ] == [ 0, 0, 0 ]", "subscriptions = always Sub.none"},
