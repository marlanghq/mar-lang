...
var keywordHoverDescriptions = map[string]string{
	...
	"screens":                      "Declares an App UI navigation model made of screens, sections, lists, links, text, and actions.",
	...
	"define-query":                 "Declares a named backend query. Parameterized queries use `(define-query (posts-by-author author-id) ...)`.",
	...
	"define":                       "Defines a named constant or function. Example: `(define app-config ...)` for a constant or `(define (square x) (* x x))` for a function.",
	"define-record":                "Declares a record type with named fields.",
	"define-type":                  "Declares a tagged union type with named variants.",
	"define-entity":                "Declares an entity. Mar generates CRUD endpoints for it. Example: `(define-entity post (fields ((body string))) ...)`.",
	"define-action":                "Declares a backend REST mutation. Example: `(define-action publish-post (input ((body string))) (create post ((body body))))`.",
