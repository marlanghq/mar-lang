...
func printUsage(binaryName string) {
	...
	fmt.Printf("  %-45s %s\n", fmt.Sprintf("%s fly destroy <app.mar>", binaryName), "Permanently destroy the Fly.io app configured for this project.")
	fmt.Printf("  %-45s %s\n", fmt.Sprintf("%s fly logs <app.mar>", binaryName), "Fetch the most recent Fly.io app logs without tailing.")
